...
- [Enterprise Agent Operations: Scale, Govern, and Monitor on Agent Runtime](./hacks/agent-runtime-ops/README.md)
  > Learn how to scale, govern, and operate autonomous AI agents in production. You will package a complete Python ADK agent using Bring Your Own Container (BYOC), deploy and scale it on Agent Runtime, enforce least-privilege access using SPIFFE-based Agent Identity, protect against prompt injections, jailbreaks, and PII leakage using Agent Gateway and Model Armor, and configure continuous quality assurance with Online Monitors.
